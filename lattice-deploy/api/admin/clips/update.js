/**
 * POST /api/admin/clips/update
 *
 * Mod-gated. Performs a clip action: remove or toggle featured.
 *
 * Body:
 *   {
 *     authToken: "<JWT>",
 *     clipUrl: "...",
 *     action: "remove" | "feature"  // feature = toggle Featured flag
 *   }
 */
...
export default async function handler(req, res) {
  ...
  const { authToken, clipUrl, action } = req.body || {};
  ...
  if (!clipUrl) {
    return res.status(400).json({ ok: false, error: "clipUrl is required." });
  }
  if (action !== "remove" && action !== "feature") {
    return res
      .status(400)
      .json({ ok: false, error: "action must be 'remove' or 'feature'." });
  }

  try {
    const result = await callAppsScript(webhookUrl, {
      action: action === "remove" ? "removeClip" : "toggleFeaturedClip",
      adminSecret,
      clipUrl,
    });
    return res.status(result.ok ? 200 : 400).json(result);
  } catch (err) {
    console.error("clip update failed:", err);
