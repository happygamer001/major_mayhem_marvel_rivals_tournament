 ...
 *
 * Records a match result. Mod-only.
 *
 * Body:
 *   {
 *     authToken: "<JWT>",
 *     matchId: "WB-R1-M1",
 *     winnerId: "team-001",
 *     teamAScore: 3 | "" | undefined,
 *     teamBScore: 1 | "" | undefined,
...
export default async function handler(req, res) {
  ...
  const { authToken, matchId, winnerId, teamAScore, teamBScore, notes } = req.body || {};
  ...
  if (!modCheck.isMod) {
    return res
      .status(403)
      .json({ ok: false, error: "Only mods can submit results." });
  }

  // Light client-side validation; Apps Script does the real validation
