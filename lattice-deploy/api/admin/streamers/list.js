/**
 * GET /api/admin/streamers/list
 *
 * Mod-gated. Returns ALL streamers (pending, approved, rejected) with
 * mod-only fields included.
 */
...
export default async function handler(req, res) {
  ...
  try {
    const result = await callAppsScript(webhookUrl, {
      action: "listAllStreamers",
      adminSecret,
    });
    return res.status(result.ok ? 200 : 502).json(result);
  } catch (err) {
    console.error("list streamers failed:", err);
