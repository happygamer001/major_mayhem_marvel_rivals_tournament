/**
 * POST /api/admin/streamers/decision
 *
 * Mod-gated. Approve or reject a streamer application.
 *
 * Body:
 *   {
 *     authToken: "<JWT>",
...
export default async function handler(req, res) {
  ...
  const { authToken, discordId, decision } = req.body || {};
  ...
  try {
    const result = await callAppsScript(webhookUrl, {
      action: decision === "approve" ? "approveStreamer" : "rejectStreamer",
      adminSecret,
      discordId,
      approvedByUsername: verified.username || verified.global_name || "",
    });
    return res.status(result.ok ? 200 : 400).json(result);
  } catch (err) {
    console.error("streamer decision failed:", err);
